...
const GOLDEN_TEXT = `page search.html  viewport 1440×900 dpr 1  full page height 1024 (needs scroll)
...
│  │  ├─ 搜索输入框 <interactive>  (320, 96, 680, 48)  grid r2 c4-8  inside:search-bar  placeholder="搜索…"  interactive · tabbable · focused
│  │  └─ 搜索按钮 <interactive>  (1008, 96, 112, 48)  grid r2 c9-9  inside:search-bar · sibling-after:search-input  type="submit"  interactive · contrast 6.8 · ⚠ 不在 Tab 顺序中  text "搜索"
│  └─ 结果列表 <group>  (320, 232, 800, 560)  grid r4 c4-9  ×12 same-structure gap 16
│     └─ 结果项 <visual>  (320, 232, 800, 64)  grid r4 c4-9  text "标题 + 摘要"…(30 chars total)
└─ 页脚 <landmark>  (0, 936, 1440, 88)  grid r16 c1-12  outside viewport

checks:
  ✓ [error] 搜索按钮在 form 内  actual: 在 form#search 内
  ✗ [error] 搜索按钮可被 Tab 选中  actual: 不在 Tab 顺序中  expected: 可 Tab
...
test('parse refuses a foreign snapshot version (no auto-migration)', () => {
  ...
  assert.throws(() => parseSnapshotJson(bytes), /render snapshot version mismatch/)
})

test('parse refuses a malformed snapshot', () => {
  assert.throws(() => parseSnapshotJson('{"version":1}'), /render snapshot malformed/)
})
