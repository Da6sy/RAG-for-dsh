...
- 进度:M0–M6 主线 ✅ · M8 ✅ · M9 存储中心化 ✅ · M9.1 面板与宿主工作区同源 ✅ · 两级知识检索 ✅(M9-0…M9-5) · 审批工作台 ✅ · **向量混合检索与精排 ✅(V0–V2)** — 288/288 测试全绿
- 两级检索一句话:**Entry 决定"相信什么"、Document 证明"原文是什么"、Chunk 决定"去哪里找"**;
  治理只在条目,原文是不可变快照,分片是可重建的派生索引(`docs/重构提案_两级知识检索.md`)
...
实现记录与实测数字、两个 dsh 契约坑见 `docs/开发记录-V3-V5-三通道与模型重排.md`。
