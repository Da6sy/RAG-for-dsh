...
- 文档**不进本仓库**:设计/历程/评测数字/待办/框架讲解都单独存放(本机在 `../clue-harness-archive-noncode/docs/`)——
  仓库里只留产品自身的代码与测试。代码注释里出现的「原规划 §X」指合并前的那几份设计文档(已并入那份存档)
- 命令入口:`clue`(对话)/ `clue web`(网页面)/ `clue render`(渲染验证)/ `clue kb`(知识库与工作区名单)
- 上游 pin:`@deepseek-ai/*@0.1.1-rc.2`(季度手动升级,compat 层收敛)
- 进度:M0–M9.1 ✅(框架/证据闭环/审批/门禁/知识循环/工作区/两级检索/面板) · V0–V5 ✅(向量混合检索与精排) · R2 ✅(一级换 BM25F) · F0–F4 ✅(评测台 + 能力门控 + 真词频/子词切分) · F1/F2/F3 档位已实现(**默认未翻**,待真端点 A/B) · R1 ✅(倒排索引:单查询 1.61s → 67ms) — 393/393 测试全绿
- 两级检索一句话:**Entry 决定"相信什么"、Document 证明"原文是什么"、Chunk 决定"去哪里找"**;
  治理只在条目,原文是不可变快照,分片是可重建的派生索引
...
> **文案语言约定**：`clue` 命令行的全部输出（含帮助、报错、`bench`/`render` 的报告）**一律英文**；
> 网页界面与喂给模型的上下文块保持中文；知识条目本身的数据（标题/正文/履历）保持原样。
> 共用字符串用渲染函数的 `lang: 'zh' | 'en'`（默认 `zh`）分流，控制台表面显式传 `'en'`。
...
真端点消融的原始指标 JSON 与截图随文档一起存放在仓库外。
